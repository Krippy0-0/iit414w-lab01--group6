...
# Two-stop strategies barely move P(top10) but can multiply P(top5) — the right call depends on the race objective
...
![Calibration curve for `is_top10` on the held-out test set.](figures/cal_top10.png)
...
![Calibration curve for `is_top5` on the held-out test set.](figures/cal_top5.png)
...
The complete codebase is available at [https://github.com/Krippy0-0/iit414w-lab01--group6](https://github.com/Krippy0-0/iit414w-lab01--group6), tagged `final-v1`. The runbook in `README.md` reproduces the modeling artifacts end-to-end in under 10 minutes on a standard Python 3.11 environment following `environment.yml` or the course environment. `RANDOM_SEED = 414` is set in every `random_state=` argument. The executed notebook `hito2/hito2_modeling.executed.ipynb` regenerates the metric tables, slice analysis, and what-if outputs; the calibration figures embedded in this report are stored under `final_report/figures/`.
